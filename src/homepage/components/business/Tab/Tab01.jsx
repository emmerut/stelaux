                        ...
                        <Tab className="tab-style-01" key={i} eventKey={i} title={item.tabTitle ? item.tabTitle : "Tab Title"}>
                            <m.div className='row items-center' {...{ ...fadeIn, transition: { duration: 0.9 }, viewport: { once: false } }}>
                                ...
                                <Col lg={{ offset: 1, span: 5 }} md={6} className="md:px-[15px] sm:px-[15px] sm:text-start xs:text-center">
                                    ...
                                    {(item.buttonLink && item.buttonTitle) && <Buttons ariaLabel="tab button" className="btn-fill text-slate-200 rounded-none font-medium font-serif tracking-[1px] uppercase btn-fancy md:mb-[15px] xs:mb-0 xs:mt-[20px]" size="md" themeColor="#232323" to={item.buttonLink ? item.buttonLink : "#"} title={item.buttonTitle} />}
                                </Col>
                            </m.div>
                        </Tab>
                    )
