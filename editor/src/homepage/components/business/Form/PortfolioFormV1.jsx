...
const MyForm = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const heroSlider = homepageDataResult.base.filter(item => item.component === 'portfolioV1');
        setData(heroSlider);
  ...
  const handleSubmit = async (values) => {
    ...
    values.inputs.forEach((input, index) => {
      ...
      formData.append(`inputs[${index}][tag]`, input.tag);
      formData.append(`inputs[${index}][component]`, 'portfolioV1');
  ...
  const initialValues = { inputs: dataForm?.length > 0 ? dataForm : [{ title: '', subtitle: '', tag: '', main_image: '', component: 'portfolio' }] };
  ...
  const validate = (values) => {
    ...
    values.inputs.forEach((input, index) => {
      ...
      if (!input.tag) {
        errors[`inputs.${index}.tag`] = 'El campo Etiqueta es requerido';
      } else if (input.tag.length > 50) {
        errors[`inputs.${index}.tag`] = 'El campo Etiqueta no puede exceder los 50 caracteres';
      }
    ...
    if (values.inputs.length < 1) {
      errors.inputs = 'Se necesitan al menos 1 formset';
    }
                ...
                {({ insert, remove, push }) => (
                  <>
                    {values.inputs.map((input, index) => (
                      ...
                      <div key={index}>
                        <Input
                          name={`inputs.${index}.tag`}
                          label={`Etiqueta ${index + 1}`}
                        />
                        {errors[`inputs.${index}.tag`] && (
                        ...
                        <button type="button" onClick={() => {push({ title: '', subtitle: '', main_image: '' });
                          }} className="mt-2">
