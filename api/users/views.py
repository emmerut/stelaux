...
class UserViewSet(viewsets.ModelViewSet):
    def send_verification(self, account_sid, auth_token, service_sid, channel, recipient, channel_configuration):
    ...
    def login(self, request):
        ...
        if user is not None:
            ...
            payload = {
                'user_id': user.id,
                'exp': timezone.now() + timezone.timedelta(days=1)  # Token expiration in 1 hour
            }
    ...
    def register(self, request):
        ...
        if serializer.is_valid():
            ...
            if user.email:
                ...
                return Response({'error': 'No se especificó canal de verificación'}, status=400)
        else:
